...
# in redundant inputs and outputs, and might result in memory allocation that
# scales unnecessarily with the branch count. This function finds common
# residual types across branches for reuse, so as to avoid redundant allocation.
def _merge_branch_residuals(branch_res_avals):
  ...
  branch_res_avals = _map(enumerate_equal, branch_res_avals)
  all_avals = sorted(set(util.concatenate(branch_res_avals)))
  all_avals_indices = {v: i for i, v in enumerate(all_avals)}
  branch_indices = [
      [all_avals_indices[aval] for aval in avals] for avals in branch_res_avals]
  all_avals = [x for x, _ in all_avals]
...
def _join_cond_outputs(jaxprs, all_res_avals, res_aval_indices_per_jaxpr,
  ...
  def augment_jaxpr(jaxpr, res_indices):
    ...
    def f_aug(*args):
      ...
      aug_residuals = _substitute(aug_residuals, res_indices, residuals)
      return outs + aug_residuals
...
def _join_cond_pe_staged_jaxpr_inputs(jaxprs, all_res_avals,
  ...
  def augment_jaxpr(jaxpr, res_indices):
    ...
    aug_res_vars = _substitute(all_res_vars, res_indices, res_vars)
...
def _substitute(xs, indices, values):
  xs = list(xs)
  assert len(indices) == len(values)
  assert all(0 <= i < len(xs) for i in indices)
